...
Balance mismatch flag triggered

Drained account flag triggered
...
A combined risk counter:

sum_of_flags_triggered = sum(all six flags)


Then fraud rate analyzed per number of triggered flags — confirming higher flags → higher fraud likelihood.
...
ROC-AUC ≈ 0.964
Excellent ranking ability between fraud and normal transactions.
...
To simulate real-world systems, final transaction decisions were created:
...
Allow	< 0.780	Remaining 99.5%	very low fraud	—	normal processing

This mirrors how real fraud engines operate:
small high-precision block list + medium review queue + majority auto-approved.
...
Thus thresholds are chosen by business cost optimization, not only ML metrics.
...
ML-only score outperformed weighted combination.
...
In real systems, rules add value only when they use external signals not present in ML features.
Since our rules were derived from same columns, ML alone captured their signal better.

This demonstrates an important real-world modeling insight.
...
This mirrors real fraud operations dashboards.
...
Block queue is highly concentrated with fraud → good precision

Review queue contains moderate fraud → analyst action justified

Allow queue almost clean → low customer friction

Exactly the desired real-world behavior.
